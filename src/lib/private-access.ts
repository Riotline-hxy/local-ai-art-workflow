...
export function accessDenied(request: NextRequest): NextResponse | null {
    ...
    if (!secret || !sameSecret(request.cookies.get(accessCookie)?.value || '', sessionValue(secret))) {
        return NextResponse.json({ error: '请使用专属邀请链接打开图片工作台。' }, { status: 403, headers: { 'Content-Type': 'application/json; charset=utf-8' } });
    }
    if (!['GET', 'HEAD', 'OPTIONS'].includes(request.method)) {
        const origin = request.headers.get('origin');
        if (origin && origin !== process.env.PHOTO_PUBLIC_ORIGIN) {
            return NextResponse.json({ error: '请求来源不受信任。' }, { status: 403, headers: { 'Content-Type': 'application/json; charset=utf-8' } });
        }
    }
    return null;
}
